...
async function verifySlackSignature(req: NextRequest, rawBody: string): Promise<boolean> {
    try {
        ...
        const hexSig = "v0=" + Array.from(new Uint8Array(signature))
...
export async function POST(req: NextRequest) {
    ...
    const isValid = process.env.SKIP_SLACK_SIGNATURE === "true" || await verifySlackSignature(req, rawBody);
    if (!isValid) {
