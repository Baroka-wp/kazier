...
function FormField({
  ...
  return (
    <div style={{ marginBottom: "10px" }}>
      ...
      {warning && !error && (
        <p style={{ marginTop: "4px", fontSize: "0.7rem", color: "#B7791F" }}>{warning}</p>
      )}
    </div>
  );
}
...
function EditModal({
  ...
  async function handleBlurCheck(
    ...
    if (result.exists) {
      if (result.blocking)
        setErrors((e) => ({
          ...e,
          [field === "full_name" ? "first_name" : field]: result.message,
        }));
  ...
  async function handleSubmit() {
    ...
    setServerError("");
    const result =
      mode === "create" ? await registerUser(values) : await updateUser(member!.id, values);
    setSaving(false);
        ...
        <div style={{ padding: "18px 22px 22px" }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "10px" }}>
            <FormField
              ...
              onBlur={() =>
                handleBlurCheck("full_name", `${values.first_name} ${values.last_name}`)
              }
            />
            <FormField
              label="Nom"
          ...
          {/* Slack ID */}
          <FormField
            label="Slack ID (optionnel)"
            value={values.slack_id}
            placeholder="U0AJC8FU64R"
      ...
      ? [
          {
            ...
            label: "Supprimer",
            onClick: (m: TeamMember) => setToDelete(m),
          },
        ]
      : []),
  ];
      ...
      <DataTable
        columns={[
            ...
            render: (m) => (
              <div style={{ display: "flex", alignItems: "center", gap: "10px" }}>
                ...
                <div>
                  ...
                  {m.slack_id && (
                    <div style={{ fontSize: "0.68rem", color: "#aaa" }}>@{m.slack_id}</div>
                  )}
                </div>
              </div>
            ),
          },
          {
            ...
            label: "Rôle",
            sortable: true,
            render: (m) => <RoleBadge role={m.role ?? "Membre"} />,
          },
          {
            key: "email",
            label: "E-mail",
            sortable: true,
            render: (m) => (
              <span style={{ fontSize: "0.8rem", color: "#555" }}>{m.email ?? "—"}</span>
            ),
          },
          ...
          { key: "age", label: "Âge", sortable: true },
          {
            ...
            render: (m) =>
              m.user_id ? (
                <span style={{ fontSize: "0.7rem", color: "#2D7A4F", fontWeight: 600 }}>
                  ✓ Actif
                </span>
              ) : (
