...
function ChangePasswordModal({ userId, onClose }: { userId: string; onClose: () => void }) {
  ...
  const strength =
    password.length === 0
      ...
      : password.length < 8
        ? 1
        : password.length >= 12 && /[\d\W]/.test(password)
          ? 3
          : 2;
          ...
          >
            ...
            <div>
              ...
              <input
                ...
                onFocus={(e) => {
                  if (passwordMatch) e.target.style.borderColor = "#6B1A2A";
                }}
                onBlur={(e) => {
                  e.target.style.borderColor =
                    confirm.length > 0 && !passwordMatch ? "#EF4444" : "rgba(0,0,0,0.1)";
                }}
              />
...
export default function Sidebar() {
  ...
  const initials = session ? `${firstName[0] ?? ""}${lastName[0] ?? ""}`.toUpperCase() || "?" : "";
  ...
  const getRoleLabel = (role: string) =>
    ({ SA: "Super Admin", TM: "Team Manager", T: "Team" })[role] ?? role;
          ...
          {visibleNavItems.map((item) => {
            ...
            return (
              <Link
                ...
                onMouseEnter={(e) => {
                  if (!isActive) e.currentTarget.style.background = "rgba(107,26,42,0.04)";
                }}
                onMouseLeave={(e) => {
