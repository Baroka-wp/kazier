  ...
  return key
    ...
    .reduce(
      (acc: Record<string, unknown> | undefined, k: string) =>
        (acc as Record<string, unknown> | undefined)?.[k] as Record<string, unknown> | undefined,
      obj
    );
}
...
export default function DataTable<T extends { id: number | string }>({
  ...
  const totalPages =
    serverTotalPages ?? Math.max(1, Math.ceil((totalItems ?? data.length) / pageSize));
                    ...
                    <td key={col.key as string} style={{ padding: "14px 20px" }}>
                      <div
                        style={{
                          ...
                          width: `${60 + (((i * columns.length + colIdx) * 7) % 30)}%`,
                        }}
                      />
                    </td>
                  ))}
                  ...
                  {columns.map((col) => (
                    <td
                      ...
                      style={{
                        ...
                        color: "#1A1A1A",
                        borderBottom:
                          rowIdx < paginated.length - 1 ? "1px solid rgba(0,0,0,0.04)" : "none",
                        verticalAlign: "middle",
                        // ✅ Contraindre la largeur si col.width est défini
                        ...(col.width
                          ? { width: col.width, maxWidth: col.width, overflow: "hidden" }
                          : {}),
                      }}
                    >
                      {col.render
          ...
          <div style={{ display: "flex", alignItems: "center", gap: "4px" }}>
            ...
            {getPageRange().map((p, i) =>
              p === "…" ? (
                <span
                  key={`ellipsis-${i}`}
                  style={{ padding: "0 4px", color: "#aaa", fontSize: "0.8rem" }}
                >
                  …
                </span>
              ) : (
                <PageBtn key={p} active={p === page} onClick={() => handlePageChange(p as number)}>
                  {p}
                </PageBtn>
              )
            )}
            <PageBtn
              onClick={() => handlePageChange(Math.min(displayTotalPages, page + 1))}
