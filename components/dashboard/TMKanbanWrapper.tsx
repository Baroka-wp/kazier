...
export default function TMKanbanWrapper({ tasks }: Props) {
  ...
  async function handleDragEnd(event: DragEndEvent) {
    ...
    // 👇 TM peut tout déplacer sauf depuis terminée
      ...
      <div style={{ display: "flex", gap: "16px", overflowX: "auto" }}>
        {(["à faire", "en cours", "review", "terminée"] as const).map((status) => (
          <TaskColumn
            ...
            teamMemberId={0} // 👈 0 = pas de filtrage par membre
            onTaskUpdated={(updatedTask) => {
              setLocalTasks((prev) => prev.map((t) => (t.id === updatedTask.id ? updatedTask : t)));
            }}
            readOnly={false} // 👈 TM peut tout modifier
            isTM={true} // 👈 nouveau flag
          />
        ))}
      </div>
