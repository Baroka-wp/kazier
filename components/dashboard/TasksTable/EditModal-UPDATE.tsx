...
function UpdateTaskForm({
  ...
  useEffect(() => {
    ...
    async function loadMembers() {
      if (task.project_id) {
        const res = await getTeamMembersByProject(task.project_id);
        if (!cancelled && res.success && res.members) {
  ...
  useEffect(() => {
    ...
    async function loadMembers() {
      ...
      const res = await getTeamMembersByProject(values.project_id);
      if (!cancelled) {
        ...
        if (res.success && res.members) {
          ...
          setValues((v) => ({
            ...v,
            assigned_to: v.assigned_to.filter((id) => validIds.includes(id)),
          }));
        }
      }
    }
  ...
  return (
    <div style={{ padding: "18px 22px 22px" }}>
      ...
      {values.project_id && (
        <div style={{ marginBottom: "10px" }}>
          <small style={labelStyle}>Assigné à{loadingMembers ? " (chargement...)" : ""}</small>
          {!loadingMembers && projectMembers.length > 0 ? (
            <>
              ...
              {values.assigned_to.length > 0 && (
                ...
                >
                  {values.assigned_to.length} personne{values.assigned_to.length > 1 ? "s" : ""}{" "}
                  sélectionnée{values.assigned_to.length > 1 ? "s" : ""}
                </p>
              )}
            </>
          ) : !loadingMembers ? (
