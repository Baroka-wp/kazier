...
function ToastNotification({ toast, onClose }: { toast: Toast; onClose: () => void }) {
  ...
  return (
    ...
    >
      ...
      <button
        onClick={onClose}
        style={{ background: "none", border: "none", cursor: "pointer", color: "#aaa", display: "flex", padding: "2px" }}
      >
        <X size={14} />
      </button>
      <style>{`
...
function EditModal({
  ...
  async function handleSubmit() {
    ...
    const result = mode === "create" ? await createProject(data) : await updateProject(project!.id, data);
  ...
  return (
    <div
      onClick={onClose}
      style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.35)", zIndex: 120, display: "flex", alignItems: "center", justifyContent: "center", padding: "20px" }}
    >
      <div
        onClick={(e) => e.stopPropagation()}
        style={{
      ...
      >
        ...
        <div style={{ padding: "18px 22px 22px" }}>
          ...
          <div style={{ marginBottom: "10px" }}>
            ...
            <input type="text" value={values.icon} placeholder="Ex: 📊" onChange={(e) => setField("icon", e.target.value)} style={inputStyle} />
          </div>
          ...
          <div style={{ marginBottom: "10px" }}>
            ...
            >
              {teams.length === 0 ? (
                ...
                teams.map((team) => {
                  const checked = values.team_ids.includes(String(team.id));
                  return (
                    <label
                      key={team.id}
                      onClick={() => {
                        const currentIds = values.team_ids.split(",").filter(Boolean);
                        const newIds = currentIds.includes(String(team.id))
                          ? currentIds.filter((id) => id !== String(team.id))
                          : [...currentIds, String(team.id)];
                        setField("team_ids", newIds.join(","));
                      }}
                      style={{
                        display: "flex",
                        alignItems: "center",
                        ...
                        padding: "4px 6px",
                        borderRadius: "8px",
                        background: checked ? "rgba(107,26,42,0.06)" : "transparent",
                    ...
                    >
                      ...
                      <span style={{ fontSize: "0.82rem", color: "#1A1A1A", fontWeight: checked ? 600 : 400 }}>{team.full_name}</span>
                    </label>
                  );
                })
              )}
...
export default function ProjectsTable({
  ...
  return (
    <>
      ...
      {toDelete && (
        <DeleteModal project={toDelete} loading={deleting} onCancel={() => setToDelete(null)} onConfirm={handleDelete} />
      )}
